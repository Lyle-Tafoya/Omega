    ...
    if (Current_Environment == E_CITY) {
      if (gamestatusp(UNDEAD_GUARDS)) {
        ...
        default:
          ...
          p_damage(random_range(Imprisonment * 10), UNSTOPPABLE,
                   "police brutality");
          morewait();
          repair_jail();
          Player.y = 54;
...
void sign_print(int x, int y, int signp) {
  ...
  case L_CHARITY:
    print1(
        "You notice a sign: The Rampart Orphanage And Hospice For The Needy.");
    break;
  case L_MANSION:
...
void l_oracle() {
  ...
  if (gamestatusp(ATTACKED_ORACLE) && (!gamestatusp(COMPLETED_ASTRAL))) {
    ...
    print1(
        "You come before a blue crystal dais. There is a bell and a mirror.");
      ...
      if (ynq2() == 'y') {
        ...
        if (!gamestatusp(SPOKE_TO_DRUID)) {
          ...
          if (ynq2() == 'y') {
            ...
            clearmsg();
            print1(
                "odd looking room whose walls seem strangely insubstantial.");
            gain_experience(5000);
            change_environment(E_ASTRAL);
          } else
...
void l_safe() {
  ...
  if (response == 'p')
    attempt =
        (2 * Player.dex + Player.rank[THIEVES] * 10 - random_range(100)) / 10;
  else if (response == 'f')
