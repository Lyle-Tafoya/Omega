...
void make_plains() { make_general_map("."); }
...
void make_forest() {
  make_general_map("\".");
  straggle_corridor(0, random_range(LENGTH), WIDTH, random_range(LENGTH), WATER,
                    RS_COUNTRYSIDE);
}

void make_jungle() { make_general_map("\"\"."); }
...
void make_swamp() { make_general_map("~~\"."); }
...
void maze_level() {
  ...
  if (Current_Environment == E_ASTRAL)
    ...
    rsi = RS_VOLCANO;
  maze_corridor(random_range(WIDTH - 1) + 1, random_range(LENGTH - 1) + 1,
                random_range(WIDTH - 1) + 1, random_range(LENGTH - 1) + 1, rsi,
                0);
...
void maze_corridor(int fx, int fy, int tx, int ty, char rsi, char num) {
  if (num < 6) {
    ...
    maze_corridor(tx, ty, random_range(WIDTH - 1) + 1,
                  random_range(LENGTH - 1) + 1, rsi, num + 1);
  }
}
