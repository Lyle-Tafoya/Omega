...
void straggle_corridor(int fx, int fy, int tx, int ty, Symbol loc, char rsi) {
  ...
  while ((fx != tx) || (fy != ty)) {
    ...
    if (random_range(abs(dx) + abs(dy)) < abs(dx))
      corridor_crawl(&fx, &fy, sign(dx), 0, random_range(abs(dx)) + 1, loc,
                     rsi);
    else
      corridor_crawl(&fx, &fy, 0, sign(dy), random_range(abs(dy)) + 1, loc,
                     rsi);
  }
}
...
void corridor_crawl(int *fx, int *fy, int sx, int sy, int n, Symbol loc,
                    char rsi) {
  int i;
  for (i = 0; i < n; i++) {
    *fx += sx;
...
void sewer_corridor(int x, int y, int dx, int dy, Symbol locchar) {
  ...
  while (continuing) {
    ...
    y += dy;
    if (locchar == WATER)
      continuing = (inbounds(x, y) && ((Level->site[x][y].locchar == WALL) ||
