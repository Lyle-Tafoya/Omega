...
void s_wish() {
  if (random_range(100) > Player.iq + Player.pow + Player.level) {
    ...
    p_damage(random_range(Spells[S_WISH].powerdrain), UNSTOPPABLE,
...
void s_firebolt() {
  ...
  setspot(&x, &y);
  fbolt(Player.x, Player.y, x, y, Player.dex * 2 + Player.level,
        Player.level * 10 + 10);
}
...
void s_missile() {
  int x = Player.x, y = Player.y;
  setspot(&x, &y);
  nbolt(Player.x, Player.y, x, y, Player.dex * 2 + Player.level,
        Player.level * 3 + 3);
}

void s_teleport() { p_teleport(0); }
...
void s_sleep() { sleep_monster(0); }

void s_heal() { heal(3); }

void s_dispel() { dispel((Player.level + Player.maxpow) / 10); }

void s_breathe() { breathe(0); }

void s_invisible() { invisible(0); }

void s_warp() { warp(1); }

void s_enchant() { enchant(1); }

void s_bless() { bless(0); }

void s_restore() { recover_stat(0); }

void s_cure() { cure(0); }

void s_truesight() { truesight(0); }
...
void s_desecrate() { sanctify(-1); }

void s_haste() { haste(0); }

void s_summon() { summon(0, -1); }

void s_sanctuary() { sanctuary(); }

void s_sanctify() { sanctify(1); }

void s_accuracy() { accuracy(0); }
...
void s_apport() { apport(0); }

void s_shadowform() { shadowform(); }

void s_alert() { alert(0); }

void s_regenerate() { regenerate(0); }

void s_clairvoyance() { clairvoyance(10); }

void s_drain() { drain(0); }

void s_levitate() { levitate(0); }

void s_polymorph() { polymorph(0); }
...
void s_identify() { identify(0); }

void s_objdet() { objdet(1); }

void s_mondet() { mondet(1); }
...
static int spell_ids[] =
    {/* in the same order as spell_names[] */
...
int spellparse() {
  ...
  do {
    ...
    if (byte == BACKSPACE || byte == DELETE) {
      ...
      l = last;
      while (f < NUMSPELLS &&
             (!Spells[spell_ids[f]].known || strlen(spell_names[f]) < pos ||
              spell_names[f][pos] < byte))
        f++;
      while (l >= 0 &&
             (!Spells[spell_ids[l]].known || strlen(spell_names[l]) < pos ||
              spell_names[l][pos] > byte))
        l--;
      if (l < f)
        continue;
      prefix[pos++] = byte;
