...
void l_adept()
{
  ...
  if (ynq2()!='y') {
    ...
    print2("and clears....");
    morewait();
    print1("You hear a voice boom:");
    print2("Welcome to the Adept's Challenge!");
    morewait();
    Player.hp = Player.maxhp;
    Player.mana = calcmana();
...
void l_vault()
{
  ...
  if ((hour()==23)) {
    ...
    Level->site[Player.x][Player.y+1].locchar = FLOOR;
  }
  else {
    print2("The door is closed.");
    Level->site[Player.x][Player.y+1].locchar = WALL;
    morewait();
    clearmsg();
