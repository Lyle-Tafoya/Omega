...
int blessing;
{
  if (blessing < 0)
    ...
    if (Player.rank[NOBILITY] > 0) {
      menuprint(" (");
      menunumprint(Player.rank[NOBILITY]);
      menuprint(ordinal(Player.rank[NOBILITY]));
...
int blessing;
{
  ...
  if (Current_Environment != Current_Dungeon) 
    ...
    if (newlevel >= MaxDungeonLevels) {
      mprint("You have been deflected!");
      newlevel=random_range(MaxDungeonLevels);
    }
