...
do_scan(sourcefp, destfp) FILE *sourcefp, *destfp;
{
  int max_strings;
  int ch, last_ch = EOF;
  ...
  string_list = (char **)malloc((max_strings = REALLOC_INCR) * sizeof(char *));
  ...
  while ((ch = getc(sourcefp)) != EOF) {
    ...
    case '"':
      if (!include_flag && last_ch != '\'') {
        /* Start of a string */
        for (temp = temp_string; (*temp = getc(sourcefp)) != '"'; temp++)
          ;
        *temp = '\0';
        strcpy(
        ...
        if (++num_strings == max_strings)
          string_list = (char **)realloc(
              string_list, (max_strings += REALLOC_INCR) * sizeof(char *));
        include_tmp = 0;
        break;
      }
    default:
      if (include_tmp) {
        if ((include_tmp = ch == include[pos++]) && pos == include_size) {
          include_flag = 1;
          include_tmp = 0;
        }
      }
...
do_output(sourcefp, destfp) FILE *sourcefp, *destfp;
{
  char buf[1024];
  int i;
