...
void l_altar()
{
  ...
  if (ynq2() == 'y') {
    if (Player.rank[PRIESTHOOD] == 0) increase_priest_rank(deity);
	...
	else increase_priest_rank(deity);
      }
    }
  }
}
...
void increase_priest_rank(deity)
int deity;
{
  ...
  case DESTINY:
    ...
    else if (Player.rank[PRIESTHOOD]== HIGHPRIEST) answer_prayer();
