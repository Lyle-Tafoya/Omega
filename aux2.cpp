...
int p_immune(int dtype) { return (Player.immunity[dtype] > 0); }
...
void p_drown() {
  ...
  if (Player.status[BREATHING] > 0)
    ...
    while (Player.possessions[O_ARMOR] ||
      ...
      case 1:
        print3(
            "You try to hold your breath... You choke... Your lungs fill...");
        break;
      case 0:
        p_death("drowning");
      }
      ...
      case 'c':
        ...
        for (i = 0; i < MAXPACK; i++) {
          if (Player.pack[i] != NULL) {
            if (Level->site[Player.x][Player.y].p_locf != L_WATER)
              p_drop_at(Player.x, Player.y, Player.pack[i]->number,
                        Player.pack[i]);
...
void change_environment(char new_environment) {
  ...
  if (((Last_Environment == E_CITY) || (Last_Environment == E_VILLAGE)) &&
    ...
    LastTownLocY = Player.y;
              ...
              (Last_Environment == E_HOUSE) || (Last_Environment == E_HOVEL) ||
