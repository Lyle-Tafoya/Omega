...
void annihilate(int blessing) {
  ...
  if (blessing > 0) {
    if (Current_Environment == E_COUNTRYSIDE) {
      ...
      morewait();
      print1(
          "There is a rain of small birds and insects from the sky, and you");
...
void learnspell(int blessing) {
  ...
  if (blessing < 0) {
    ...
    if ((random_range(4 * Spells[spell].powerdrain) +
         Spells[spell].powerdrain) < (4 * Player.iq + 8 * Player.level)) {
  ...
  if (blessing > -1) {
    if (r > ROOMBASE) {
      if (loc_statusp(Player.x, Player.y, LIT, *Level))
        ...
        spreadroomlight(Player.x, Player.y,
    ...
    if (r > ROOMBASE) {
      if (!loc_statusp(Player.x, Player.y, LIT, *Level))
        ...
        spreadroomdark(Player.x, Player.y,
                       Level->site[Player.x][Player.y].roomnumber);
      }
    } else
