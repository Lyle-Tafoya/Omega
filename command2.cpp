...
void pickup() {
  if (Level->site[Player.x][Player.y].things == NULL)
    ...
    print3(
        "You can't really interact with the real world in your shadowy state.");
  else
  ...
  if (item == CASHVALUE)
    ...
    if (Player.str + random_range(20) > obj->fragility + random_range(20)) {
      ...
      if (obj->objchar == WEAPON) {
        ...
        p_damage(random_range(obj->dmg + abs(obj->plus)), NORMAL_DAMAGE,
...
void save(int compress, int force) {
  ...
  if (force || ok) {
    ...
    for (pos = 0; fname[pos] && isprint(fname[pos]) && !isspace(fname[pos]);
      ...
      ;
    if (fname[pos]) {
      sprintf(Str1, "Illegal character '%c' in filename - Save aborted.",
              fname[pos]);
      print1(Str1);
      ok = FALSE;
    }
