...
void l_altar() {
  ...
  if (ynq2() == 'y') {
    if (Player.rank[PRIESTHOOD] == 0)
      ...
      while ((response != 'b') && (response != 's') && (response != 'p') &&
             (response != ESCAPE));
      if (response == 'b') {
        ...
        if (Blessing) {
          ...
          morewait();
          gain_experience(Player.rank[PRIESTHOOD] * Player.rank[PRIESTHOOD] *
                          50);
          cleanse(1);
          heal(10);
          bless(1);
        ...
        if (Player.possessions[i] == NULL) {
          ...
          p_damage(Player.hp - 1, UNSTOPPABLE, "a god's pique");
                   ...
                   (long)(Player.rank[PRIESTHOOD] * Player.rank[PRIESTHOOD] *
...
void hp_req_print() {
  ...
  case DRUID:
    print2(
        "any of the aligned priests who may be found in their main Temples.");
    break;
  case DESTINY:
